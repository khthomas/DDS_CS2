...
author: "Matt Rega, Steven Hayden, and Kyle Thomas"
...
#####Gets users Working directory and adds the file path to the data file####
My_FilePath<-getwd()
...
#imports raw data from csv file from the above path
...
#The code below scrapes data from the tables on the wiki page in the URL below. 
...
#change the column names so that the dataframes can be bound together
...
#Output HDI table in CSV format into the data directory. 
write.csv(dfHDI, file = paste(My_FilePath,"/Data/HDI_Table.csv",sep = ""),row.names=FALSE)
...
#Output HDI table in CSV format into the data directory. 
write.csv(Procrastination, file = paste(My_FilePath,"/Data/Clean_Procrastination_Table.csv",sep = ""),row.names=FALSE)
...
Below is the descriptive statistics of the data. The descriptive statistics describe the basic features of the data. A function has been created for WDC convince. In the future WDC may see the need to have descriptive statistics on other variables. The descriptive report function can take any variable inputted into and out the median, min, max, mean, standard deviation, and range. 
...
```{r}
#This code block passes variables of inters into descriptive  report 
#function to create descriptive statics data frame
...
Histograms are a useful tool to understand the shape of your data to get an idea if any transformation/ assumption are needed for the analyses. 
...
### Summary Statistics table and Frequency tables 
...
countryGPmean = aggregate(GPMean ~ Country + HDI_Category + `2016_Est_HDI`, Procrastination, mean)
sortedGP = countryGPmean[order(countryGPmean$GPMean, decreasing = TRUE),][1:15,]

#Q6C
#Output HDI table in CSV format into the data directory. 
write.csv(sortedGP, file = paste(My_FilePath,"/Data/Top15MostProcrastinatingCountries_GPMean.csv",sep = ""),row.names=FALSE)
...
#Q5C Barchart
countryDPmean = aggregate(DPMean ~ Country + HDI_Category +`2016_Est_HDI`, Procrastination, mean)
sortedDP = countryDPmean[order(countryDPmean$DPMean, decreasing = TRUE),][1:15,]


#Q6C
#Output HDI table in CSV format into the data directory. 
write.csv(sortedDP, file = paste(My_FilePath,"/Data/Top15MostProcrastinatingCountries_DPMean.csv",sep = ""),row.names=FALSE)
