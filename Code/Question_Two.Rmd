...
frequency_Table<-arrange(frequency_Table,desc(frequency_Table$`Count by Occupation`))


#Outputs the full occupation list into the data folder 
write.csv(frequency_Table, file = paste(My_FilePath,"/Data/count_by_Occupation_list.csv",sep = ""),row.names=FALSE)

#outputs the top 10
kable(head(frequency_Table,10),format='html',output=F)  %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
```
Top 10
...
#Outputs the full occupation list into the data folder 
write.csv(frequency_Table, file = paste(My_FilePath,"/Data/count_by_Country_list.csv",sep = ""),row.names=FALSE)
